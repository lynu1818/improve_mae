...
from .hiera import Hiera, HieraBlock, HieraSTMoE, HieraBlockSTMoE
...
class MaskedAutoencoderHieraSTMoE(HieraSTMoE):
    """Masked Autoencoder with HieraSTMoE backbone"""
    ...
    def __init__(
        ...
        **kwdargs,
        ...
        # Use simple Hiera for decoder regardless of ST-MoE
        self.decoder_blocks = nn.ModuleList(
            [
                HieraBlock(
                    dim=decoder_embed_dim,
...
@pretrained_model({}, default=None)
def mae_hiera_base_plus_512(**kwargs):
    return MaskedAutoencoderHiera(
...
@pretrained_model({})
def mae_hiera_base_plus_512_st_moe_0011_50p(**kwargs):
    moe_stages = (
        False, False,
        False, False, False,
        False, False, False, False, False, False, False, False,
    ...
    assert len(moe_stages) == sum(stages)
    return MaskedAutoencoderHieraSTMoE(
        embed_dim=112, num_heads=2, stages=stages, moe_stages=moe_stages, q_pool=2, num_experts=16, input_size=(512, 512), **kwargs,
    )
