...
class MaskedAutoencoderPlusViT(nn.Module):
    ...
    def forward_decoder(self, x, ids_shuffle):
        ...
        B, N, D = x.shape
        N -= 1  # remove cls token
        ids_keep = ids_shuffle[:, :N]

        # detach cls token from decoder_pos_embed
        expanded_pos_embed = self.decoder_pos_embed[:, 1:, :].expand(x.shape[0], -1, -1)
        expanded_cls_token = self.decoder_pos_embed[:, :1, :].expand(x.shape[0], -1, -1)
